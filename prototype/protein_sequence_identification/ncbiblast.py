...
parser.add_option(
    "--program",
    type=str,
    help=("The BLAST program to be used for the Sequence Similarity Search."),
)
parser.add_option(
    "--task", type=str, help=("Task option (only selectable for blastn)")
)
parser.add_option(
    "--matrix",
    type=str,
    help=(
        "(Protein searches) The substitution matrix used for scoring alignments"
...
parser.add_option(
    "--alignments",
    type=int,
    help=("Maximum number of match alignments reported in the result output."),
)
parser.add_option(
    "--scores",
    type=int,
...
parser.add_option(
    ...
    type=str,
    help=(
        "Limits the number of scores and alignments reported based on the"
...
parser.add_option(
    ...
    type=int,
    help=(
        "The amount a score can drop before gapped extension of word hits is"
        "halted"
    ),
)
parser.add_option(
    ...
    type=str,
    help=(
        "(Nucleotide searches) The match score is the bonus to the alignment"
...
parser.add_option(
    ...
    type=int,
    help=(
        "Maximum number of HSPs (alignments) to keep for any single query-"
        "subject pair."
    ),
)
parser.add_option(
...
parser.add_option(
    ...
    type=str,
    help=(
        "Filter regions of low sequence complexity. This can avoid issues with"
...
parser.add_option(
    ...
    action="store_true",
    help=(
        "This is a true/false setting that tells the program the perform"
...
parser.add_option(
    "--wordsize", type=int, help=("Word size for wordfinder algorithm")
)
parser.add_option(
    "--taxids",
    type=str,
    help=(
        "Specify one or more TaxIDs so that the BLAST search becomes"
        "taxonomically aware."
    ),
)
parser.add_option(
    "--negative_taxids",
    type=str,
    help=("TaxIDs excluded from the BLAST search."),
)
parser.add_option(
    "--compstats", type=str, help=("Use composition-based statistics.")
)
parser.add_option("--align", type=int, help=("Formating for the alignments"))
parser.add_option(
    "--transltable",
    type=int,
    help=("Query Genetic code to use in translation"),
)
parser.add_option(
    "--stype",
    type=str,
    help=("Indicates if the sequence is protein or DNA/RNA."),
)
parser.add_option(
    "--sequence",
    type=str,
    help=(
        "The query sequence can be entered directly into this form. The"
...
parser.add_option(
    "--multifasta",
    action="store_true",
    help="Treat input as a set of fasta formatted sequences.",
)
parser.add_option(
    "--useSeqId",
    action="store_true",
    help="Use sequence identifiers for output filenames."
...
parser.add_option(
    "--debugLevel", type="int", default=debugLevel, help="Debugging level."
)
...
def restRequest(url):
    ...
    try:
        ...
        if (
            len(resp) > 0
            and contenttype != u"image/png;charset=UTF-8"
...
def serviceGetParameterDetails(paramName):
    ...
    printDebugMessage(
        u"serviceGetParameterDetails", u"paramName: " + paramName, 2
    )
    requestUrl = baseUrl + u"/parameterdetails/" + paramName
    printDebugMessage(
        u"serviceGetParameterDetails", u"requestUrl: " + requestUrl, 2
    )
    xmlDoc = restRequest(requestUrl)
    doc = xmltramp.parse(xmlDoc)
...
def serviceRun(email, title, params):
    ...
    try:
        ...
        reqH = urlopen(
            req, requestData.encode(encoding=u"utf_8", errors=u"strict")
        )
...
def serviceGetResultTypes(jobId):
    ...
    printDebugMessage(
        u"serviceGetResultTypes", u"requestUrl: " + requestUrl, 2
    )
    xmlDoc = restRequest(requestUrl)
    doc = xmltramp.parse(xmlDoc)
